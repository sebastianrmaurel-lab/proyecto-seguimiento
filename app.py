...
class Contrato(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'etapa': self.etapa, 'estado': self.estado,
            'pago_imprevisto': self.pago_imprevisto, 'visado': self.visado,
...
def api_chart_data():
    ...
    return jsonify({
        'by_month': by_month, 'etapas': etapas,
...
def api_resp_stats(nombre):
    ...
    return jsonify({'total': total, 'en_proceso': en_proceso, 'completado': completado,
                    'sin_efecto': sin_efecto, 'criticos': criticos, 'advertencias': advertencias,
                    'visados': visados, 'con_obs': con_obs, 'contratos': contratos})
...
def api_crear_responsable():
    ...
    if Responsable.query.filter_by(nombre=nombre).first():
        return jsonify({'error': 'Ya existe un responsable con ese nombre'}), 400
